...
const EnterAmountForm: FC<EnterAmountFormProps> = ({ formData, setFormData }) => {
  ...
  const setSelectedCurrency = (currency: string) => {
    setFormData({ ...formData, payment_currency_upper: currency.toUpperCase() });
  };

  const setAmountToRequest = (amount: number) => {
    setFormData({ ...formData, payment_amount: amount });
  };
  ...
  function fileToBase64(file: File): Promise<string> {
    return new Promise((resolve, reject) => {
      ...
      reader.onload = () => {
        if (reader.result) {
          // Ensure result is a string, then strip the Base64 prefix
      ...
      <div className="flex flex-col justify-start items-start w-3/4 gap-6">
        ...
        <div className="flex flex-col justify-start items-start w-full gap-1.5">
          <div className="flex flex-col justify-start items-start w-full relative gap-1.5">
            ...
            <div
              className="flex justify-start items-center self-stretch flex-grow-0 flex-shrink-0"
            ...
            >
              <div className="flex justify-start items-center flex-grow relative">
                <input
                  ...
                  onChange={(e) => setAmountToRequest(parseFloat(e.target.value))}
                  className={`w-4/5 px-3.5 py-2.5 border ${
                    formData.has_amount_error ? "border-red-400" : "border-primary-50"
                  } focus:border-primary focus:outline-none focus:ring-0 rounded-l-xl text-base text-left text-dark-blue-hue`}
                  placeholder="Amount"
                />
                <select
                  value={formData.payment_currency_upper}
                  onChange={(e) => setSelectedCurrency(e.target.value)}
                  className="w-1/5 rounded-r-xl bg-white dark:text-primary px-3.5 py-2.5 border border-primary-50 focus:outline-none"
                >
            ...
            >
              <div className="flex justify-start items-center flex-grow relative gap-2">
                <input
                  ...
                  className="flex-grow w-full px-3.5 py-2.5 border border-primary-50 focus:border-primary focus:outline-none focus:ring-0 rounded-xl text-base text-left text-dark-blue-hue"
