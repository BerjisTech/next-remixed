...
const SignInPage = () => {
  ...
  const handlePasswordValidation = async (email_or_username: string, password: string) => {
    setLoading(true);
    try {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (!isIdentifierValid) {
      ...
      // await handlePasswordValidation(email_or_username, password);
        ...
        <div className="p-6 bg-white rounded-lg shadow-lg flex-col align-middle justify-items-center">
          ...
          <form className="mt-6 w-full" onSubmit={handleSubmit}>
            ...
            <Button
              ...
              className={`block w-full mt-4 bg-primary text-white py-2 px-4 rounded-md hover:bg-primary-700 ${loading ? "opacity-50 cursor-not-allowed" : ""}`}
              disabled={loading}
            >
              {loading ? "Validating..." : isIdentifierValid ? "Sign in" : "Continue"}
            </Button>
          </form>
          ...
          <div className="text-center mt-4">
            ...
            <p className="mt-2 text-sm">
              ...
              <a href="/next/register" className="text-indigo-500 hover:underline">
                Sign-up
              </a>
            </p>
          </div>
